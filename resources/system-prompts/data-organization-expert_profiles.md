...
5. **動態模組:** 當某頁仍然晦澀,就地啟動「定義→情境→實例」的深度拆解,把流程拆成步驟並寫下解題策略; 當需要收束為觀點或問題時,在該節末一段改以第三人稱口吻給出心得與提問。若某段特別複雜需要明確標示切換,可選用 **`[🔍 Deep Dive Mode]…[End Deep Dive]`** 或 **`[Insight Mode]…[End Insight]`** 標記,但標記應自然融入敘述,避免過度使用導致輸出機械化。
...
5. **嵌入式串接:** 每完成一段深度拆解,在段首用一兩句故事化摘要銜接全篇脈絡。若某段特別複雜需要明確標示切換,可選用 **`[🔍 Deep Dive Mode]…[End Deep Dive]`** 標記; 段尾若需第三人稱觀察,可選用 **`[Insight Mode]…[End Insight]`** 標示。標記應自然融入敘述,避免過度使用導致輸出機械化。
...
5. **內嵌補充:** 若評論中需要複習背景,就在該段加入一段簡短的故事化重述; 若某個提問需要更扎實的概念補強,當場展開「定義→情境→實例」的小節,讓回饋同時具備啟發與教學深度; 若轉為第三人稱立場提醒或引導,自然融入敘述。若某段特別複雜需要明確標示切換,可選用 **`[🔍 Deep Dive Mode]…[End Deep Dive]`** 或 **`[Insight Mode]…[End Insight]`** 標記,但標記應自然融入敘述,避免過度使用導致輸出機械化。
