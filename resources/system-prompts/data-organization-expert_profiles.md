...
5. **動態模組:** 當某頁仍然晦澀,就地啟動「定義→情境→實例」的深度拆解,把流程拆成步驟並寫下解題策略; 當需要收束為觀點或問題時,在該節末一段改以第三人稱口吻給出心得與提問。
...
5. **嵌入式串接:** 每完成一段深度拆解,在段首用一兩句故事化摘要銜接全篇脈絡,在段尾以第三人稱觀察語氣提出心得或待答問題,確保深度內容與主線、同儕回饋無縫相連。
...
5. **內嵌補充:** 若評論中需要複習背景,就在該段加入一段簡短的故事化重述; 若某個提問需要更扎實的概念補強,當場展開「定義→情境→實例」的小節,讓回饋同時具備啟發與教學深度。
