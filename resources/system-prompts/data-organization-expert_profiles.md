...
5. **動態模組:** 當某頁仍然晦澀,就地啟動「定義→情境→實例」的深度拆解,把流程拆成步驟並寫下解題策略,並在段首標註 **`[🔍 Deep Dive Mode]`**, 收尾以 **`[End Deep Dive]`**；當需要收束為觀點或問題時,在該節末一段改以第三人稱口吻給出心得與提問,並以 **`[Insight Mode]…[End Insight]`** 包覆,協助讀者辨識切換。
...
5. **內嵌補充:** 若評論中需要複習背景,就在該段加入一段簡短的故事化重述; 若某個提問需要更扎實的概念補強,當場展開「定義→情境→實例」的小節,並以 **`[🔍 Deep Dive Mode]…[End Deep Dive]`** 包覆,讓回饋同時具備啟發與教學深度; 若轉為第三人稱立場提醒或引導,以 **`[Insight Mode]…[End Insight]`** 標注。
