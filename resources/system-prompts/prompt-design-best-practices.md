...
❌ **不良設計：**
```
直接要求：「對每個概念進行原子級拆解，使用 [🔍 Deep Dive Mode] 標記」
結果：AI 直接跳入細節，缺少整體脈絡
```
...
**問題：** 強制要求每段都使用標記（如 `[🔍 Deep Dive Mode]`），導致輸出機械化。

**解決方案：** 將標記改為可選，僅在特別複雜需要明確標示切換時使用。
...
✅ **良好設計：**
```
「當某頁仍然晦澀，就地啟動深度拆解。若某段特別複雜需要明確標示切換，可選用 [🔍 Deep Dive Mode]…[End Deep Dive] 標記，但標記應自然融入敘述，避免過度使用。」
結果：標記只在必要時使用，輸出更自然
```
...
2. **標記過度使用**：每段都強制使用 `[🔍 Deep Dive Mode]` 標記，導致輸出機械化
...
**教訓：** 標記（如 `[🔍 Deep Dive Mode]`）可以幫助讀者辨識切換，但過度使用會讓輸出感覺機械化。

**實踐：** 將標記改為可選，只在特別複雜需要明確標示時使用。
